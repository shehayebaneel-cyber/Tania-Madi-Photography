...
import { api, mediaUrl, type PortfolioItem } from "../lib/api";
      ...
      <section>
        <div className="wrap">
          ...
          {loading ? <div className="spinner" /> : items.length === 0 ? <div className="empty"><h3>No photos in this category yet</h3></div> : (
            <div className="pgrid">
              {items.map((p, i) => <Tone key={p.id} tone={p.tone} img={p.imageUrl ? mediaUrl(p.imageUrl) : undefined} className={`cell ${span(i)}`} label={p.category.replace(/-/g, " ")} seed={p.id} onClick={() => setIdx(i)} />)}
            </div>
          )}
        </div>
      </section>
